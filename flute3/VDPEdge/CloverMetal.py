...
for file_name in file_names:
    ...
    if (pos!=-1):
        ...
        if label_contents[-1]=="s":
            ...
            ax1.title.set_text("VDPEdge"+" : "+label_contents[2]+"_"+label_contents[3]+"_"+"standard")
            ##fit the data
            ...
            ax2.title.set_text("VDPEdge"+" : "+label_contents[2]+"_"+label_contents[3]+"_"+"rotated")
            ##fit the data
            # position_current_fit1 = int(np.where(current == -9.000000e-08)[0]) #position in which the fit starts
            pars1, cov = optimize.curve_fit(fit_func, current[3:],
                                            voltage[3:])  # pars1 is the parameters of the fit and cov is the convolution
...
file_exists = os.path.isfile('./VDPEdge.csv')
...
with open('VDPEdge.csv', newline='', mode='a') as csv_file:
    fieldnames = ['Name', 'VDPEdge-standard [Ohm/square]', 'VDPEdge-standard-err [Ohm/square]',
                  'VDPEdge-rotated [Ohm/square]','VDPEdge-rotated-err [Ohm/square]']
    writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
    if not file_exists:
        print("Write header")
        writer.writeheader()  # file doesn't exist yet, write a header
    writer.writerow({'Name': label_list[0], 'VDPEdge-standard [Ohm/square]': Rsh_s_list[0],
                     'VDPEdge-standard-err [Ohm/square]' : deltaRsh_s_list[0],
                     'VDPEdge-rotated [Ohm/square]': Rsh_r_list[0],
                    'VDPEdge-rotated-err [Ohm/square]' : deltaRsh_r_list[0]})
    #written_data = pd.read_csv("./VDPEdge.csv", sep=",")
    for i in range(1,len(label_list)):
        #if label_list[i] not in written_data.values:
        print(label_list[i])
        writer.writerow({'Name': label_list[i], 'VDPEdge-standard [Ohm/square]': Rsh_s_list[i],
                         'VDPEdge-standard-err [Ohm/square]': deltaRsh_s_list[i],
                         'VDPEdge-rotated [Ohm/square]': Rsh_r_list[i],
                         'VDPEdge-rotated-err [Ohm/square]': deltaRsh_r_list[i]})


#manager = plt.figure(1).get_current_fig_manager()
#manager.resize(*manager.window.maxsize())
#plt.figure(1).savefig('VDPEdge-standard.png')
#manager = plt.get_current_fig_manager()
#manager.resize(*manager.window.maxsize())
#fig2.savefig('VDPEdge-rotated.png')
#plt.show()
...
fig1.savefig('VDPEdge-standard.png',dpi=300)
fig2.savefig('VDPEdge-rotated.png',dpi=300)

plt.show()
#fig2.savefig('VDPEdge-rotated.png',bbox_inches='tight',pad_inches=0.1)
