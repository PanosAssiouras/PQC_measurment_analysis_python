...
for file_name in file_names:
    ...
    if (pos!=-1):
        ...
        ax1.title.set_text("CCPoly"+" : "+label_contents[2]+"_"+label_contents[3])
        ##fit the data
       # position_current_fit1 = int(np.where(current == (-1.8E-6))[0]) #position in which the fit starts
        ...
        file_exists = os.path.isfile('./CCPoly.csv')
        print("file_exists=",file_exists)
        with open('CCPoly.csv', newline='',mode='a') as csv_file:
            ...
            if not file_exists:
                ...
                written_data=pd.read_csv("./CCPoly.csv",sep=",")
...
fig1.savefig('CCPoly.png',dpi=300)
