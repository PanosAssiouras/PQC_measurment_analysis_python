...
for file_name in file_names:
    ...
    if (pos!=-1):
        ...
        ax1.title.set_text("CCStrip"+" : "+label_contents[2]+"_"+label_contents[3])
        ##fit the data
       # position_current_fit1 = int(np.where(current == (-1.8E-6))[0]) #position in which the fit starts
        ...
        file_exists = os.path.isfile('./CCStrip.csv')
        print("file_exists=",file_exists)
        with open('CCStrip.csv', newline='',mode='a') as csv_file:
            ...
            if not file_exists:
                ...
                written_data=pd.read_csv("./CCStrip.csv",sep=",")
...
fig1.savefig('CCStrip.png',dpi=300)
