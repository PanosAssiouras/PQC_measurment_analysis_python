...
path_par=PurePath(path.parents[2], 'flute1/VDPPoly')
...
data_VDPPoly=pd.read_csv(Rsh_data_file_names[0], sep=",")
VDP_name_r=[]
VDP_name_s=[]
Rsh_s=[]
Rsh_r=[]
delta_Rsh_s=[]
delta_Rsh_r=[]
for index, row in data_VDPPoly.iterrows():
    #print(measurment)
    #name=data_VDPPoly.iloc[index:,0]
    name=row[0]
    if "_r" in name:
        #print(measurment)
        Rsh_r.append(row[1])
...
data_VDPPoly_r=pd.DataFrame(data=d)
d = {'VDP_name_s': VDP_name_s, 'Rsh_s': Rsh_s,'delta_Rsh_s':delta_Rsh_s }
data_VDPPoly_s = pd.DataFrame(data=d)

print(data_VDPPoly_r)
print(data_VDPPoly_s)
...
for file_name in file_names:
    ...
    if (pos!=-1):
        ...
        if label_contents[-1] == "r":
            for measurment in data_VDPPoly_r['VDP_name_r']:
                if label_check in measurment:
                    if label not in label_list:
                        label_list.append(label)
                    listOfPositions = getIndexes(data_VDPPoly_r,measurment)
                    #print(type(listOfPositions))
                    flat_1 = [x for l in listOfPositions for x in l]
                    #print(flat_1[0])
                    #print(flat_1[1])
                    Rsh_r=data_VDPPoly_r.at[flat_1[0],'Rsh_r']
                    dRsh_r=data_VDPPoly_r.at[flat_1[0],'delta_Rsh_r']
                    ...
                    ax2.title.set_text(
                        "CBKR-Poly" + " : " + label_contents[2] + "_" + label_contents[3] + "_" + "rotated")
                    ##fit the data
        ...
        if label_contents[-1] == "s":
            for measurment in data_VDPPoly_s['VDP_name_s']:
                if label_check in measurment:
                    if label not in label_list:
                        label_list.append(label)
                    listOfPositions = getIndexes(data_VDPPoly_s,measurment)
                    #print(type(listOfPositions))
                    flat_1 = [x for l in listOfPositions for x in l]
                    #print(flat_1[0])
                    #print(flat_1[1])
                    Rsh_s=data_VDPPoly_s.at[flat_1[0],'Rsh_s']
                    dRsh_s=data_VDPPoly_s.at[flat_1[0],'delta_Rsh_s']
                    name = label_contents[4] + "_" + label_contents[1] + "_" \
                           + label_contents[6] + "_" + label_contents[8] + "_" + "standard"
                    ax1.plot(current, voltage, linestyle='solid', marker='o', label=name)
                    ax1.title.set_text(
                        "CBKR-Poly" + " : " + label_contents[2] + "_" + label_contents[3] + "_" + "standard")
                    ##fit the data
                    # position_current_fit1 = int(np.where(current == -9.000000e-08)[0]) #position in which the fit starts
                    pars1, cov = optimize.curve_fit(fit_func, current[3:],
                                                    voltage[
                    ...
                    ######## Write results to CBKRPoly.csv ####################################

        label =" "

######## Write results to VDPPoly.csv ####################################
print(label_list)
...
fig1.savefig('CBKRPoly-standard.png',dpi=300)
fig2.savefig('CBKRPoly-rotated.png',dpi=300)
