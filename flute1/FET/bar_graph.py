...
for file_name in file_names:
    ...
    if (pos!=-1):
        ...
        measurment_contents=col_names[0].split('_')
        print(measurment_contents)
        print(os.path.basename(__file__))
        for (columnName, columnData) in data.iteritems():
           # print('Colunm Name : ', columnName)
           # print(type(columnData.values[0]))
           name_conents = col_names[index].split('_')
           if type(columnData.values[0]) == str:
               print("{} It is string".format(columnName))
                ...
                ax.bar(name_conents[4]+" "+name_conents[5]+name_conents[8],col_names[index], 0.8,
                       alpha=0.5, ecolor='black', capsize=10, label='rotated')
                #ax.hist(columnData.values,
                #        range=[(round_down(min(columnData.values),digits+1)-
                #                0.1 * abs(round_down(min(columnData.values),digits+1))),
                #               round_up(max(columnData.values),digits+1)+
                #               0.1* abs(round_up(max(columnData.values),digits+1))],
                #        bins='scott', alpha=0.5, color='blue', edgecolor='white', linewidth=1.2)
                fig.set_size_inches(width / 100, height / 100)
                fig.savefig("BarHisto"+"_"+col_names[index].split(' ')[0]+"_"+measurment+".png", dpi=100)
        index+=1
plt.show()
