...
for file_name in file_names:
    ...
    if (pos!=-1):
        ...
        if label_contents[-1]=="s":
            ...
            ax1.title.set_text("VDPPoly"+" : "+label_contents[2]+"_"+label_contents[3]+"_"+"standard")
            ##fit the data
            ...
            ax2.title.set_text("VDPPoly"+" : "+label_contents[2]+"_"+label_contents[3]+"_"+"rotated")
            ##fit the data
            # position_current_fit1 = int(np.where(current == -9.000000e-08)[0]) #position in which the fit starts
            pars1, cov = optimize.curve_fit(fit_func, current[3:],
                                            voltage[3:])  # pars1 is the parameters of the fit and cov is the convolution
...
######## Write results to VDPPoly.csv ####################################
        name=label_contents[3]+'_'+label_contents[4]+'_'+label_contents[2]+'_'+\
             label_contents[1]+'_'+label_contents[6]+ '_' + label_contents[9]
        file_exists = os.path.isfile('./VDPPoly.csv')
        print("file_exists=",file_exists)
        with open('VDPPoly.csv', newline='', mode='a') as csv_file:
            fieldnames = ['Name', 'R_sh [Ohm/square]', 'delta_Rsh [Ohm/square]']
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
            if not file_exists:
                print("Write header")
                writer.writeheader()  # file doesn't exist yet, write a header
                writer.writerow({'Name': name, 'R_sh [Ohm/square]': Rsh, 'delta_Rsh [Ohm/square]': deltaRsh})
            else:
                written_data=pd.read_csv("./VDPPoly.csv",sep=",")
                if name not in written_data.values:
                    writer.writerow({'Name': name, 'R_sh [Ohm/square]': Rsh, 'delta_Rsh [Ohm/square]': deltaRsh})


#manager = plt.figure(1).get_current_fig_manager()
#manager.resize(*manager.window.maxsize())
#plt.figure(1).savefig('VDPPoly-standard.png')
#manager = plt.get_current_fig_manager()
#manager.resize(*manager.window.maxsize())
#fig2.savefig('VDPPoly-rotated.png')
#plt.show()
...
fig1.savefig('VDPPoly-standard.png',dpi=300)
fig2.savefig('VDPPoly-rotated.png',dpi=300)

plt.show()
#fig2.savefig('VDPPoly-rotated.png',bbox_inches='tight',pad_inches=0.1)
