        ...
        for (columnName, columnData) in data.iteritems():
           ...
           # print(type(columnData.values[0]))
            if type(columnData.values[0])==str:
                ...
                ax = fig.add_subplot()
                #ax.set_xlim(-1.5*max(columnData.values),1.5*max(columnData.values))
                ...
                ax.hist(columnData.values,
                       range=[(round_down(min(columnData.values),digits+1)-
                                1.5 * abs(round_down(min(columnData.values),digits+1))),
                               round_up(max(columnData.values),digits+1)+
                               1.5* abs(round_up(max(columnData.values),digits+1))],
                        bins='scott', alpha=0.5, color='blue', edgecolor='white', linewidth=1.2)
                #ax.hist(columnData.values,
                #        range=[-1.0*round_down(max(columnData.values), digits + 1),
                #               1.0*round_up(max(columnData.values), digits + 1)],
                #        bins='scott', alpha=0.5, color='blue', edgecolor='white', linewidth=1.2)
                #ax.hist(columnData.values,bins='scott', alpha=0.5, color='blue', edgecolor='white', linewidth=1.2)
