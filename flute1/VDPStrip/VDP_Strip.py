...
for file_name in file_names:
    ...
    if (pos!=-1):
        ...
        if label_contents[-1]=="s":
            ...
            ax1.title.set_text("VDPStrip"+" : "+label_contents[2]+"_"+label_contents[3]+"_"+"standard")
            ##fit the data
            ...
            ax2.title.set_text("VDPStrip"+" : "+label_contents[2]+"_"+label_contents[3]+"_"+"rotated")
            ##fit the data
            # position_current_fit1 = int(np.where(current == -9.000000e-08)[0]) #position in which the fit starts
            pars1, cov = optimize.curve_fit(fit_func, current[3:],
                                            voltage[3:])  # pars1 is the parameters of the fit and cov is the convolution
...
##### Write results to VDPStrip.csv ####################################
        ...
        file_exists = os.path.isfile('./VDPStrip.csv')
        print("file_exists=",file_exists)
        with open('VDPStrip.csv', newline='', mode='a') as csv_file:
            ...
            if not file_exists:
                ...
                written_data=pd.read_csv("./VDPStrip.csv",sep=",")
...
#plt.figure(1).savefig('VDPStrip-standard.png')
#manager = plt.get_current_fig_manager()
#manager.resize(*manager.window.maxsize())
#fig2.savefig('VDPStrip-rotated.png')
#plt.show()
...
fig1.savefig('VDPStrip-standard.png',dpi=300)
fig2.savefig('VDPStrip-rotated.png',dpi=300)

plt.show()
#fig2.savefig('VDPStrip-rotated.png',bbox_inches='tight',pad_inches=0.1)
