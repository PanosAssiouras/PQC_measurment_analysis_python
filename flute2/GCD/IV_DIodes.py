...
def optimizefit_horizontal_curve_with_zero_slope(xdata,ydata,start_posfit1,start_posfit2):
    ...
    #print(start_posfit2-1.0)
    position_voltage_horizontal_fit1 =int(np.where(xdata == (start_posfit2-0.2))[0])
    #print(position_voltage_horizontal_fit1)
    position_voltage_horizontal_fit2 = int(np.where(xdata == (start_posfit2))[0])
    #print(position_voltage_horizontal_fit2)
    r_squared={}
    intercepts={}
...
q=1.602E-7*1E-12 #pAs charge unit
#Area=0.125*0.125 -math.pi*(math.pow((0.04/2.0),2)) #cm^2 Diode Area of flute1
A=(0.1093*0.0870)-(10*(0.003*0.1)); #cm^2 Diode Area of flute2
k=1.38064852*1E-23
T=273+24.7      #J/K
...
for file_name in file_names:
    ...
    if (pos!=-1):
        ...
        ax1.title.set_text("IV_GCD" + " : " + label_contents[2] + "_" + label_contents[3])


        data=pd.read_csv(file_name, sep="\t",skiprows=4)
        file_name = os.path.splitext(file_name)[0]
        data_sorted=data.sort_values(by=['Voltage (V)'],ascending=True)
        data_sorted.reset_index(drop=True,inplace=True)
        voltage = data_sorted.iloc[:,0]
        current = data_sorted.iloc[:,1]
        print(voltage)
       ...
       # type(voltage)
        ...
        intercept1,\
        position_voltage_horizontal_slope0_fit1, \
        position_voltage_horizontal_slope0_fit2 = optimizefit_horizontal_curve_with_zero_slope(voltage, current,-1.0,3.0)
        ax1.plot(voltage[int(position_voltage_horizontal_slope0_fit1):int(position_voltage_horizontal_slope0_fit2)],
                 fit_func0(voltage[int(position_voltage_horizontal_slope0_fit1):int(position_voltage_horizontal_slope0_fit2)],
                           intercept1),
                 linestyle='--', linewidth=2, color='black')

        intercept2,\
        position_voltage_horizontal_slope0_fit1, \
        position_voltage_horizontal_slope0_fit2 = optimizefit_horizontal_curve_with_zero_slope(voltage, current,-7.5,-2.5)
        ...
        ax1.plot(voltage[int(position_voltage_horizontal_slope0_fit1):int(position_voltage_horizontal_slope0_fit2)],
                 fit_func0(
                     voltage[int(position_voltage_horizontal_slope0_fit1):int(position_voltage_horizontal_slope0_fit2)],
                     intercept2),
                 linestyle='--', linewidth=2, color='black')

        I_surf=intercept2-intercept1
        print(I_surf)
        S_0=I_surf/(q*ni*A)
        ...
        file_exists = os.path.isfile('./IV_GCD.csv')
        with open('IV_GCD.csv', newline='', mode='a') as csv_file:
            fieldnames = ['Name', 'S_0 [cm/s]']
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
            if not file_exists:
                writer.writeheader()  # file doesn't exist yet, write a header
                writer.writerow({'Name': label, 'S_0 [cm/s]': '%.2E' % S_0 })
            else:
                written_data = pd.read_csv("./IV_GCD.csv", sep=",")
...
fig.savefig('IV_GCD.png',dpi=300)
