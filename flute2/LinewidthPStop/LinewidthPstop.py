...
path_par=PurePath(path.parents[1], 'flute1/VDPPstop')
           ...
           #         measurement[1] + '_' + measurement[6] + '_' + measurement[8]
            ...
            if(label_check in measurement):
                ...
                if "_s" in measurement:
                    ...
                    ax1.title.set_text(
                        "LinewidthPstop" + " : " + label_contents[2] + "_" + label_contents[3] + "_" + "standard")
                    ##fit the data
                   ...
                   # label = file_name[0:pos].split('/')[-1]
                    ...
                    ax2.title.set_text(
                        "LinewidthPstop" + " : " + label_contents[2] + "_" + label_contents[3] + "_" + "rotated")
                    # position in which the fit starts
                    pars1, cov = optimize.curve_fit(fit_func, current[3:], voltage[3:])  # pars1 is the parameters of the fit and cov is the convolution
                    ax2.plot(current[3:],
                             fit_func(current[3:], *pars1),
...
file_exists = os.path.isfile('./LinewidthPstop.csv')
print("file_exists=", file_exists)
with open('LinewidthPstop.csv', newline='', mode='a') as csv_file:
    fieldnames = ['Name', 'Linewidth_Pstop-standard [m]', 'Linewidth_Pstop-rotated [m]']
    writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
    if not file_exists:
        print("Write header")
        writer.writeheader()  # file doesn't exist yet, write a header
    writer.writerow({'Name': label_list[0], 'Linewidth_Pstop-standard [m]': t_s_list[0],
                    'Linewidth_Pstop-rotated [m]': t_r_list[0]})
    #written_data = pd.read_csv("./LinewidthPstop.csv", sep=",")
    print(len(label_list))
    for i in range(1,(len(label_list))):
        #if label_list[i] not in written_data.values:
        print(label_list[i])
        writer.writerow({'Name': label_list[i], 'Linewidth_Pstop-standard [m]': t_s_list[i],
                        'Linewidth_Pstop-rotated [m]': t_r_list[i]})
...
fig1.savefig('LinewidthPstop-standard.png',dpi=300)
fig2.savefig('LinewidthPstop-rotated.png',dpi=300)
